import './index.css'


const Physicians = () => {


    return (
      <>
        <div className="container">
          <div className="physician-section2">
            <h2>Our Physicians</h2>
            <ul>
              <li>
                <a href="/physicians/james-chang">
                  <img
                    src="http://cpadvancedimaging.com/wp-content/uploads/jchang.jpg"
                    alt="James Chang"
                  />
                  <h3>James Chang, M.D., FACR</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/calvin-lo">
                  <img
                    src="http://cpadvancedimaging.com/wp-content/themes/cpai/images/calvinlo.jpg"
                    alt="James Chang"
                  />
                  <h3>Calvin Lo, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/heather-osbourne">
                  <img
                    src="http://cpadvancedimaging.com/wp-content/themes/cpai/images/hosborne.jpg"
                    alt="James Chang"
                  />
                  <h3>Heather L. Osborne, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/pouneh-fallahi">
                  <img
                    src="http://cpadvancedimaging.com/wp-content/uploads/pfallahi.jpg"
                    alt="James Chang"
                  />
                  <h3>Pouneh Fallahi, M.D. MPH</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/sabrina-pieroni">
                  <img
                    src="https://cpadvancedimaging.com/wp-content/uploads/2016/08/sabrina-pieronic-website-picture-682x1024.jpg"
                    alt="James Chang"
                  />
                  <h3>Sabrina Pieroni, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/amy-law">
                  <img
                    src="https://cpadvancedimaging.com/wp-content/uploads/2019/11/amy-law-website-pic-683x1024.jpg"
                    alt="James Chang"
                  />
                  <h3>Amy S. Law, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/robert-lautin">
                  <img
                    src="http://cpadvancedimaging.com/wp-content/themes/cpai/images/rlautin.jpg"
                    alt="James Chang"
                  />
                  <h3>Robert F. Lautin, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/hari-nandu">
                  <img
                    src="https://cpadvancedimaging.com/wp-content/uploads/2019/11/hari-nandu-website-pic-683x1024.jpg"
                    alt="James Chang"
                  />
                  <h3>Hari Nandu, M.D</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
              <li>
                <a href="/physicians/lawrence-cabusora">
                  <img
                    src="https://cpadvancedimaging.com/wp-content/uploads/2016/08/lawrence-cabusora-website-picture-682x1024.jpg"
                    alt="James Chang"
                  />
                  <h3>Lawrence Cabusora, M.D.</h3>
                  <p>Board Certified by the American Board of Radiology</p>
                </a>
              </li>
            </ul>
          </div>
        </div>
      </>
    );
}

export default Physicians